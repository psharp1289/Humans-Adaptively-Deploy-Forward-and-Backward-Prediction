...
legend = plt.legend([legend_handles[idx] for idx in order],
                    [labels[idx] for idx in order],
                    loc='upper center',
                    bbox_to_anchor=(0.5, 0),  # Adjust these values as needed
                    title="Transition Probabilities",
                    ncol=len(transitions))

# Set the title of the plot and adjust position
plt.title('State Space Study 1 Before Map Change', pad=-60)  # Adjust the pad value as needed
...
plt.savefig('State Space 1 Pre Map', dpi=300, bbox_extra_artists=(legend,), bbox_inches='tight')
plt.show()
