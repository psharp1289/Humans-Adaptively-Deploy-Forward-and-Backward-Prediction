...
for icon_name, (x, y, width, height) in icon_positions.items():
	...
	for i, (transition, probability) in enumerate(probabilities[f"{icon_name}_transitions"]):
		...
		#     head_width=0.02, head_length=0.02,linewidth=4.0, fc=arrow_color, ec=arrow_color,length_includes_head=True,zorder=1
		# )
		if strength>0.4:
...
legend = plt.legend(handles=base_rate_handles,
		   loc='upper center',
		   bbox_to_anchor=(0.5, 0.97),  # Adjust these values as needed
		   title="Base Rates",
		   ncol=2,
...
for text in legend.get_texts():
	...
	ax.plot([0.60, 0.65],[0.9, 0.92],color='black',linewidth=1,linestyle='-',zorder=8)
	ax.plot([1.00, 0.95],[0.9, 0.92],color='black',linewidth=1,linestyle='-',zorder=8)
