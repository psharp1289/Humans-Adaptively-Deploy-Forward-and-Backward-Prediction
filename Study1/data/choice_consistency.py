...
subs_csv=[['sub','PR Evidence','bias_lowbaserate','bias_finalstate','pre_map_change','post_map_change','first_half_postmap','second_half_postmap']]
...
for df in sub_dfs: #iterate over subjects
	...
	current_row=[sub_name,evidence_predecessorRepresentation/8.0,bias_low_baserateediate/8.0,distal_bias_score/8.0,pre_map_change_score/4.0,post_map_change_score/4.0,first_pm/2.0,second_pm/2.0]
	subs_csv.append(current_row)
	sub_num+=1



with open('all_planning_questions_baseratebias_choice_consistency.csv','w') as f:
	writer=csv.writer(f)
