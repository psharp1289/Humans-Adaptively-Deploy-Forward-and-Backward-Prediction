...
subs_csv=[['sub','PR Evidence','bias_highbaserate','pre_map_change','post_map_change','two_step_planning','query_counts_PR','query_counts_BR','first_half','second_half']]
			...
			if np.isfinite(df[response_mb][row]):
				...
				if type_sub==2:
					if counter_planning<13:
						if int(df[response_mb][row])!=2:
							if int(df[response_mb][row])!=8:
								num_high_br+=1
					else:
						if int(df[response_mb][row])!=2:
							if int(df[response_mb][row])!=8:
