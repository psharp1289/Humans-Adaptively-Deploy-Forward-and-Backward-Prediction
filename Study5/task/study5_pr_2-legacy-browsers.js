...
let expName = 'study5_pr_2';  // from the Builder filename that created this script
let expInfo = {
    'participant': '',
    'session': '001',
};
...
async function experimentInit() {
  ...
  instructions_rewardstage = new visual.TextStim({
    ...
    text: 'Now it’s time to use what you learned to get rewards. Each time you enter the picture world to win money, you will be told which images will give you points! \n\nYou need to use this information plan and choose the sequence of TWO ACTIONS that’s most likely to get you points. First select whether you choose the trident or planet, and then the next best action to get you to reward.\n\nPress Space to continue ',
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], height: 0.045,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
  ...
  instructions_rewardstage_2 = new visual.TextStim({
    ...
    text: 'When you enter your action, first select 1 or 9 to get start from planet and trident, which will get you to a new image based on what you learned. Then select 2 or 8 to get from the image you expect to reach on to the final image with points.\n\nYou must plan this out all based on what you learned without seeing any new outcomes.\n\nPress Space to continue ',
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], height: 0.045,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
  ...
  text_84 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_84',
    text: 'Congrats! Your bonus points will be calculated soon and delivered on Prolific!',
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], height: 0.06,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
...
function learning_trial_step2_to_step3RoutineBegin(snapshot) {
  return async function () {
    ...
    learning_trial_step2_to_step3Components.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function prediction5RoutineBegin(snapshot) {
  return async function () {
    ...
    prediction5Components.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function instructions_planning_2phaseRoutineBegin(snapshot) {
  return async function () {
    ...
    instructions_planning_2phaseComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function choice_transition2RoutineBegin(snapshot) {
  return async function () {
    ...
    choice_transition2Components.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
