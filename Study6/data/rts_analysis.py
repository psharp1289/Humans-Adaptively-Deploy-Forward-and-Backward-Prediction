...
for df in sub_dfs:
		...
		if np.isfinite(df[response][row]):
			...
			if counter_planning<9:
				...
				if q_type=='successorRepresentation':
					if int(df[response][row])!='a' and int(df[response][row])!='k':
	...
	for i in range(len(rts_planning)):
		current_row=[sub_name,evidence_successorRepresentation/4.0,base_rate_high/4.0,rr_score/4.0,tr_score/4.0,rts_planning[i],np.median(avg_RTs),trial_counter[i],planning_qs[i],pr_invidiual_qs[i],4]
