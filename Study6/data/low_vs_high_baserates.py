...
subs_csv=[['sub','SR Evidence','bias_highbaserate','pre_map_change','post_map_change','twostep_planning','num_correct_sr','num_correct_br','high_br','low_br']]
...
for df in sub_dfs:
			...
			if counter_planning<11:
				...
				if str(int(df[im2][row])) in mb_responses:
					if int(df[response][row])==rr_dict[str(int(df[im2][row]))][1]:
						rr_score+=1
						if rr_dict[str(int(df[im2][row]))][1]<3:
							high_br+=1
						else:
							low_br+=1
	...
	current_row=[sub_names[sub_num],evidence_successorRepresentation/num_correct_sr,base_rate_high/num_correct_br,rr_score/4.0,tr_score/4.0,twostep_planning_score/4.0,num_correct_sr,num_correct_br,high_br/2.0,low_br/2.0]
	if total_correct>5:
