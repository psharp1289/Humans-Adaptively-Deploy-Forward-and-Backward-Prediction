...
# Define the new transitions for "transition stage 2"
 ['trident']*2 + ['planet']*8 , # left action
 ['trident']*6 + ['planet']*4,  # right action
['trident']*2 + ['planet']*8 , # left action
['trident']*6 + ['planet']*4 , # right action
 ['trident']*8 + ['planet']*2 , # left action
 ['trident']*4 + ['planet']*6  ,# right action
['trident']*6 ,# left action
 ['trident']*6 , # right action
 ['planet']*6  ,# left action
 ['planet']*6]  # right action
...
'bell':10,'tree':12,'watch':14,'fox':16,'unicorn':18,
...
for key, val in state_transitions_dictionary.items():
    if val != 'end':
        ...
        if int(val)>9:
            probabilities['{}_transitions_otheraction'.format(key)]=[]

            transitions_temp=transitions[val+1]

            counts_dict = list_to_counts_dict(transitions_temp)
            sum_counts=sum(counts_dict.values())

            for key_i, val_i in counts_dict.items():
                probabilities['{}_transitions_otheraction'.format(key)].append([key_i,round((val_i/sum_counts),2)])
...
icon_positions = {
    ...
    'bell': (0.70, 0.55, 0.1, 0.1),
    'tree': (0.40, 0.55, 0.1, 0.1),
    'watch': (0.90, 0.55, 0.1, 0.1),
    'fox': (1.10, 0.55, 0.1, 0.1),
    'unicorn': (1.40, 0.55, 0.1, 0.1),

    # 'bell_other': (0.70, 0.5, 0.1, 0.1),
    # 'tree_other': (0.40, 0.5, 0.1, 0.1),
    # 'watch_other': (0.90, 0.5, 0.1, 0.1),
    # 'fox_other': (1.10, 0.5, 0.1, 0.1),
    # 'unicorn_other': (1.40, 0.5, 0.1, 0.1),
...
double_actions=['bell','tree','watch','fox','unicorn']
...
for icon_name, (x, y, width, height) in icon_positions.items():
    ...
    for i, (transition, probability) in enumerate(probabilities[f"{icon_name}_transitions"]):
        ...
        if icon_name in double_actions:
            target_icon_position = icon_positions[transition]
